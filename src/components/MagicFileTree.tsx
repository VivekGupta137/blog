        ...
        return (
            ...
            >
                {node.children.map((child) =>
                    renderTreeNode(child, onFileSelect)
                )}
            </Folder>
        );
    } else {
...
const MagicFileTree = ({ data, className = "" }: MagicFileTreeProps) => {
    ...
    return (
        <div
            className={`magic-file-tree relative flex h-96 w-full flex-col overflow-hidden rounded-lg border bg-background shadow-sm transition-all duration-300 hover:shadow-md ${className}`}
        >
            ...
            <div className="flex-1 overflow-hidden">
                <Tree
                    ...
                    indicator={true}
                    openIcon={
                        <FolderOpenIcon className="size-4 text-blue-600 folder-icon" />
                    }
                    closeIcon={
                        <FolderIcon className="size-4 text-slate-600 folder-icon" />
                    }
                >
            ...
            {selectedFile && (
                <div className="border-t bg-muted/20 px-4 py-2 animate-in slide-in-from-bottom-2 duration-200">
                    <p className="text-xs text-muted-foreground">
                        Selected:{" "}
                        <span className="font-mono text-foreground bg-muted/50 px-1 rounded">
                            {selectedFile}
                        </span>
                    </p>
                </div>
